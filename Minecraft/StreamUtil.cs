...
public static class StreamUtil
{
    ...
    public static TagType GetTagType(this Stream stream)
        => (TagType)stream.GetByte();

    public static Tag GetTag(this Stream stream, TagType type)
    {
        return type switch
        {
            TagType.End => EndTag.FromStream(stream),
            TagType.Byte => ByteTag.FromStream(stream),
            TagType.Short => ShortTag.FromStream(stream),
            TagType.Int => IntTag.FromStream(stream),
            TagType.Long => LongTag.FromStream(stream),
            TagType.Float => FloatTag.FromStream(stream),
            TagType.Double => DoubleTag.FromStream(stream),
            TagType.ByteArray => ByteArrayTag.FromStream(stream),
            TagType.String => StringTag.FromStream(stream),
            TagType.List => ListTag.FromStream(stream),
            TagType.Compound => CompoundTag.FromStream(stream),
            TagType.IntArray => IntArrayTag.FromStream(stream),
            TagType.LongArray => LongArrayTag.FromStream(stream),
