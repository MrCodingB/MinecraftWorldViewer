...
public class StreamUtilTests
{
    ...
    [Fact]
    public void PeeksAndThrowsWhenEmpty()
    {
        var stream = new MemoryStream(new byte[] { 14, 145, 65 });

        stream.Peek().Should().Be(14);
        stream.Peek().Should().Be(14);
        stream.Peek(2).Should().BeEquivalentTo(new byte[] { 14, 145 });
        stream.Peek(3).Should().BeEquivalentTo(new byte[] { 14, 145, 65 });
